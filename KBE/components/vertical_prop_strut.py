...
from components.load_patch_v import LoadPatchVertical
...
class Strut(Base):
    ...
    position= Input(XOY)
    color= Input('white')
    material_density = Input(1600.0)  # kg/m³ — carbon fibre default
    ...
    def inertia(self):
        ...
        I_lat = (1 / 12) * m * (3 * r**2 + h**2)  # Ixx = Iyy (lateral)
        I_ax  = 0.5 * m * r**2                      # Izz (axial)
...
class StrutCluster(Base):
    ...
    vertical_propulsor_loc = Input(0.0)
    ...
    def _radius_true(self):
        return self.envelope_radius * math.sqrt(1 - self.vertical_propulsor_loc**2)
    ...
    def _sub_strut_angle(self, index):
        ...
        if self.cluster_index in (0, 1, 2, 3):
            ...
            vertical_to_equator = (
                self.vertical_propulsor_loc * self.envelope_radius - self._attach_height
            )
            geometric_correction = math.atan2(
                horizontal_to_sphere,
                max(vertical_to_equator, 1e-6)
            )

            inner= 1.0 - (1 * self.strut_size + self.strut_radius) / self.envelope_radius
            loc_90 = math.sqrt(1.0 - inner**2)
            geometric_correction2 = (math.pi / 4) * (self.vertical_propulsor_loc / loc_90)
...
class StrutSupport(Base):
    ...
    def _cluster_position(self, index):
        tx = (
            self._radius_true  if index == 0 else
            -self._radius_true if index == 1 else
            0
        )
        ty = (
            self._radius_true  if index == 2 else
            -self._radius_true if index == 3 else
            0
        )
        rx = (
            -math.pi / 2 if index == 2 else
             math.pi / 2 if index == 3 else
            0
        )
        ry = (
            -math.pi / 2 if index == 0 else
             math.pi / 2 if index == 1 else
            0
        )
        return (
            self.position
            ...
            .translate("y",  self.vertical_propulsor_loc * self.envelope_radius if index == 0 else 0)
            .translate("y",  self.vertical_propulsor_loc * self.envelope_radius if index == 1 else 0)
            .translate("y",  self.vertical_propulsor_loc * self.envelope_radius if index == 2 else 0)
            .translate("y", -self.vertical_propulsor_loc * self.envelope_radius if index == 3 else 0)
        )
