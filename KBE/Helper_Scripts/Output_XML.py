...
def write_prop_xml(xlsx_path: str, output_name: str = "prop_airshipH") -> None:
    """
    Read J, Ct, Cp columns from xlsx_path and write a JSBSim propeller XML.
    The file at xlsx_path must have columns: J, Ct, Cp (header row 0).
    Output is written to  <kbe_root>/Airship/Engines/<output_name>.xml
    """
...
def write_flight_gear_xml(airship) -> None:
    ...
    keel_z = -(
        r +
        float(airship.payload_height) +
        float(airship.battery_height)
    )
